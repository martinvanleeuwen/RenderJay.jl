...
abstract type Asset{T<:AbstractFloat, S<:Integer} end


struct Hook{T<:AbstractFloat, S<:Integer} <: Asset{T, S}
    oidx::S
    pidx::S
    xoff::T
    yoff::T
    ...
    beta::T
    gamma::T
    Hook{T,S}(oidx::S, pidx::S, x::T, y::T, z::T, beta::T, gamma::T) where {T,S} = new(oidx, pidx, x, y, z, beta, gamma)
end
Hook(oidx::Integer, pidx::Integer, x::AbstractFloat, y::AbstractFloat, z::AbstractFloat, beta::AbstractFloat, gamma::AbstractFloat) = Hook{typeof(x), typeof(oidx)}(oidx,pidx,x,y,z,beta,gamma)


struct Transform{T<:AbstractFloat, S<:Integer} <: Asset{T, S}
    oidx::S
    pidx::S
    matrix::Array{T, 2}
    inverse::Array{T, 2}
    function Transform{T,S}(oid::S, pid::S, m::AbstractArray{T, 2}) where {T,S}
        matrix = m
	inverse = inv(m)
        oid = oid
        pid = pid
        new(oid, pid, matrix, inverse)
    end
end
Transform(oid::S, pid::S, matrix::AbstractArray{T, 2}) where {T<:AbstractFloat, S<:Integer} = Transform{eltype(matrix), typeof(oid)}(oid, pid, matrix)
...
struct Walls{T<:AbstractFloat}
    ...
    ysize::T
    zsize::T
    function Walls{T}(BSW::Point{T}, TNE::Point{T}) where {T<:AbstractFloat}
        # BSW = Bottom South West corner; TNE = Top North East corner
        bottom = BSW.z
        top = TNE.z
        north = TNE.y
        south = BSW.y
        ...
        ysize = north - south
        zsize = top - bottom
        new(bottom, top, north, south, east, west, xsize, ysize, zsize)
    end
end
Walls(p::Point{T}, q::Point{T}) where {T<:AbstractFloat} = Walls{T}(p,q)
